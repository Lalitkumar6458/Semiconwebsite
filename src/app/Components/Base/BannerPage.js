...
const BannerPage = ({heading,bredcrum}) => {
  ...
  <div className='flex items-center gap-3 justify-center'>
  {
    bredcrum?.map((item,index)=>{
        ...
        <Link className='flex items-center text-[15px] md:text-[18px] gap-2 font-semibold cursor-pointer' href={item.link}>{item.icon&&item.icon}{item.name}</Link>{bredcrum.length-1!==index?"/":null}
        </div>
    })
  }
  </div>
